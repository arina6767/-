from aiogram import Bot, Dispatcher, executor, types
import random

TOKEN = "8564961413:AAFcKuTm4WZQbG3h25_jLBvTrSPh4xp6AdI"

bot = Bot(token=TOKEN)
dp = Dispatcher(bot)

user_scores = {}
user_state = {}

tax_data = {
    "Италия": {
        "Туризм": {
            "Налог на использование флага Италии в декоре": 0,
            "Туристический сбор за проживание": 2,
            "Сбор за вход в перегруженные районы": 5,
            "Налог на сохранение исторического центра": 8
        },
        "Экология": {
            "Налог на одноразовые стаканчики": 0,
            "Сбор за пластиковые бутылки": 2,
            "Сбор за переработку отходов": 5,
            "Налог на въезд авто в центр города": 8
        },
        "Медицина": {
            "Налог на покупку витаминов туристами": 0,
            "Сбор на поддержку местных клиник": 2,
            "Сбор на профилактику заболеваний": 5,
            "Обязательный медсбор для туристов": 8
        },
        "Социальная сфера": {
            "Налог на уличных музыкантов": 0,
            "Сбор за торговлю в туристических местах": 2,
            "Налог на бизнес в исторических районах": 5,
            "Налог на коммерческое использование символов": 8
        }
    },

    "Япония": {
        "Туризм": {
            "Налог на фото в метро": 0,
            "Туристический сбор в отелях": 2,
            "Сбор на содержание храмов": 5,
            "Налог на посещение перегруженных зон": 8
        },
        "Экология": {
            "Налог на пластиковые пакеты": 0,
            "Сбор за упаковку": 2,
            "Сбор за переработку электроники": 5,
            "Налог на превышение потребления энергии": 8
        },
        "Медицина": {
            "Налог на сладости": 0,
            "Налог на сладкие напитки": 2,
            "Сбор на профилактику заболеваний": 5,
            "Налог на вредные продукты": 8
        },
        "Социальная сфера": {
            "Налог на поздние прогулки": 0,
            "Сбор за шум ночью": 2,
            "Сбор с компаний за переработки": 5,
            "Налог за нарушение баланса труда и отдыха": 8
        }
    },

    "США": {
        "Туризм": {
            "Налог на селфи в парке": 0,
            "Сбор за парковку у достопримечательностей": 2,
            "Туристический сбор в отелях": 5,
            "Налог на посещение национальных парков": 8
        },
        "Экология": {
            "Налог на пластиковые трубочки": 0,
            "Сбор за вывоз мусора": 2,
            "Налог на выбросы предприятий": 5,
            "Углеродный налог": 8
        },
        "Медицина": {
            "Налог на фастфуд": 0,
            "Налог на сладкие напитки": 2,
            "Сбор на профилактику ожирения": 5,
            "Налог на ультраобработанную пищу": 8
        },
        "Социальная сфера": {
            "Налог на рекламу": 0,
            "Налог на соцсети": 2,
            "Налог на пустующее жильё": 5,
            "Налог на спекуляцию недвижимостью": 8
        }
    },

    "Франция": {
        "Туризм": {
            "Налог на фотографирование улиц": 0,
            "Туристический сбор за музеи": 2,
            "Налог на проживание в отелях": 5,
            "Налог на авиаперелёты внутри страны": 8
        },
        "Экология": {
            "Налог на пластиковые пакеты": 0,
            "Сбор за переработку отходов": 2,
            "Налог на въезд авто в центр": 5,
            "Налог на загрязнение воздуха": 8
        },
        "Медицина": {
            "Налог на сладости": 0,
            "Налог на сладкие напитки": 2,
            "Сбор на профилактику заболеваний": 5,
            "Налог на вредные продукты": 8
        },
        "Социальная сфера": {
            "Налог на модную одежду": 0,
            "Сбор на мероприятия": 2,
            "Налог на бизнес в исторических районах": 5,
            "Налог на пустующее жильё": 8
        }
    },

    "Китай": {
        "Туризм": {
            "Налог на фото туристов": 0,
            "Туристический сбор": 2,
            "Сбор за посещение популярных зон": 5,
            "Налог на массовые туристические маршруты": 8
        },
        "Экология": {
            "Налог на упаковку": 0,
            "Сбор за переработку отходов": 2,
            "Налог на выбросы предприятий": 5,
            "Налог на промышленное загрязнение": 8
        },
        "Медицина": {
            "Налог на сладости": 0,
            "Налог на сладкие напитки": 2,
            "Сбор на профилактику заболеваний": 5,
            "Налог на вредные привычки": 8
        },
        "Социальная сфера": {
            "Налог на ночные развлечения": 0,
            "Сбор на порядок в городе": 2,
            "Сбор за перенаселённость мегаполиса": 5,
            "Налог на перегруз инфраструктуры": 8
        }
    }
}

bonus_codes = [
    "LIT2S9KGJH9",
    "LIT2Sbmkjh",
    "CLASSIC35vx52p",
    "promokodi30j",
    "PROKACHAYSYA"
]

def countries_keyboard():
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("Италия", "Япония")
    kb.add("США", "Франция", "Китай")
    kb.add("📊 Мои баллы")
    return kb

def sectors_keyboard():
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("Туризм", "Экология")
    kb.add("Медицина", "Социальная сфера")
    kb.add("⬅️ Назад к странам")
    return kb

def taxes_keyboard(country, sector):
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)

    taxes = list(tax_data[country][sector].keys())
    random.shuffle(taxes)  # ✅ перемешиваем только кнопки

    kb.add(taxes[0], taxes[1])
    kb.add(taxes[2], taxes[3])
    kb.add("⬅️ Назад к сферам")
    return kb


@dp.message_handler(commands=["start"])
async def start(message: types.Message):
    uid = message.from_user.id
    user_scores[uid] = 0
    user_state[uid] = {}
    await message.answer(
        "🌍 Игра «Необычные налоги мира»!\n"
        "Выбирай страну → сферу → налог.\n"
        "🎁 50 баллов = бонусы!",
        reply_markup=countries_keyboard()
    )


@dp.message_handler(lambda m: m.text == "📊 Мои баллы")
async def my_score(message: types.Message):
    uid = message.from_user.id
    await message.answer(f"📊 Ваши баллы: {user_scores.get(uid, 0)}", reply_markup=countries_keyboard())


@dp.message_handler(lambda m: m.text in tax_data.keys())
async def choose_sector(message: types.Message):
    uid = message.from_user.id
    user_state[uid] = {"country": message.text}
    await message.answer("Выберите сферу:", reply_markup=sectors_keyboard())


@dp.message_handler(lambda m: m.text == "⬅️ Назад к странам")
async def back_to_countries(message: types.Message):
    await message.answer("Выберите страну:", reply_markup=countries_keyboard())


@dp.message_handler(lambda m: m.text in ["Туризм", "Экология", "Медицина", "Социальная сфера"])
async def choose_tax(message: types.Message):
    uid = message.from_user.id
    if uid not in user_state or "country" not in user_state[uid]:
        await message.answer("Сначала выберите страну.", reply_markup=countries_keyboard())
        return

    user_state[uid]["sector"] = message.text
    country = user_state[uid]["country"]
    sector = user_state[uid]["sector"]

    await message.answer("Выберите налог:", reply_markup=taxes_keyboard(country, sector))


@dp.message_handler(lambda m: m.text == "⬅️ Назад к сферам")
async def back_to_sectors(message: types.Message):
    uid = message.from_user.id
    if uid not in user_state or "country" not in user_state[uid]:
        await message.answer("Выберите страну:", reply_markup=countries_keyboard())
        return
    await message.answer("Выберите сферу:", reply_markup=sectors_keyboard())


@dp.message_handler(lambda m: True)
async def handle_tax(message: types.Message):
    uid = message.from_user.id
    if uid not in user_state or "country" not in user_state[uid] or "sector" not in user_state[uid]:
        await message.answer("Сначала выберите страну и сферу.", reply_markup=countries_keyboard())
        return

    country = user_state[uid]["country"]
    sector = user_state[uid]["sector"]

    if message.text not in tax_data[country][sector]:
        await message.answer("Выберите налог с кнопок.", reply_markup=taxes_keyboard(country, sector))
        return

    points = tax_data[country][sector][message.text]
    user_scores[uid] += points

    text = (
        f"✅ Вы выбрали:\n{message.text}\n\n"
        f"🎯 Баллы: {points}\n"
        f"📊 Всего: {user_scores[uid]}"
    )

    if user_scores[uid] >= 50:
        text += "\n\n🎁 Бонусы открыты:\n" + "\n".join(bonus_codes)

    await message.answer(text, reply_markup=countries_keyboard())


if __name__ == "__main__":
    executor.start_polling(dp)
